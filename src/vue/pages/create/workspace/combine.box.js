...
function initCombine(eqxElementsSelected, eqxPage) {
    ...
    if (checkCondition(eqxElementsSelected)) {
        ...
        chooseElementArr.map(eqxElement => {
            if (eqxElement.elementJson.type === elementType.table) {
                eqxElement.isSelected = false
                Vue.notifier.warn('table组件不可参与编组哦！')
            }
        })
        // 过滤锁定的元素和table元素
        chooseElementArr = chooseElementArr.filter(element => (!element.elementJson.property.lock && element.elementJson.type !== elementType.table))
        // 过滤掉临时组合的元素
