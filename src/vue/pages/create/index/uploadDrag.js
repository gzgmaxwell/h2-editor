...
export default {
    bind($el, binding) {
        const { upload, artqrcode, editor, tcloud, puzzle } = binding.value
        const options = getOptions(upload, artqrcode, editor, tcloud, puzzle)
        ...
        onCopy = e => {
            ...
            if (Vue.store.state.scene.eqxElementsSelected.length) {
 ...
 * @param {*} upload
 */
function getOptions(upload, artqrcode, editor, tcloud, puzzle) {
    const options = {
                ...
                .then(() => {
                    if (isEnd) {
                        if (artqrcode) { // 艺术二维码-粘贴上传
                            ...
                            // 字云编辑器nav上传-粘贴上传
                            Vue.store.commit('TCLOUD_NAV', { item: tcloud.navItem })
                            Vue.store.commit('TCLOUD_NAV_PANEL', { show: true })
                        } else if (puzzle) {
                            // 拼图编辑器nav上传-粘贴上传
                            Vue.store.commit('PUZZLE_NAV', { item: puzzle.navItem })
