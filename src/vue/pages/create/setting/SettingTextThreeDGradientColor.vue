...
export default {
    ...
    watch: {
        model: {
            handler(newVal) {
                const { textGradient } = this.eqxElement.elementJson.property
                if (textGradient) {
                    this.backgroundImage = `linear-gradient(90deg, ${textGradient.colors[0]} 0%, ${textGradient.colors[1]} 100%)`
                }
            },
            deep: true
        }
    ...
    methods: {
        selsectColor(item) {
            ...
            if (this.model[this.modelKey]) {
                ...
                this.model[this.modelKey].angle = 0
                this.model[this.modelKey].change = true
            }
        },
        switchColorDirection() {
            const { textGradient } = this.eqxElement.elementJson.property
            let angle = textGradient.angle
            angle += 90
            if (angle >= 360) {
                angle -= 360
            }
            this.eqxElement.updateGradient({ textGradient: { angle, colors: textGradient.colors } })
        }
    }
}
</script>
