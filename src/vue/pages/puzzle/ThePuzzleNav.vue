...
export default {
    ...
    created() {
        Vue.store.commit('PUZZLE_NAV', { item: this.nav.list[0] })
    },
    methods: {
        switchPanel() {
            Vue.store.commit('PUZZLE_NAV_PANEL', { show: !this.navPanel.show })
        },
        onSelectNav(item) {
            Vue.store.commit('PUZZLE_NAV', { item })
        },
        onItemMouseOver(item) {
            const className = window.event.target.className
            if (window.event.target.nodeName === 'LI' && className.search('active') === -1) {
                document.querySelector('div.eqc-puzzle-nav > ul > li.item.active').style.color = '#4f5d69'
                document.querySelector('div.eqc-puzzle-nav > ul > li.item.active > span.triangle').style.display = 'none'
        ...
        onItemMouseLeave(item) {
            const className = window.event.target.className
            if (window.event.target.nodeName === 'LI' && className.search('active') === -1) {
                document.querySelector('div.eqc-puzzle-nav > ul > li.item.active').style = ''
                document.querySelector('div.eqc-puzzle-nav > ul > li.item.active > span.triangle').style.display = 'block'
            }
        }
    }
}
