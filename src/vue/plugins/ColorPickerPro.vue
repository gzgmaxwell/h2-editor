    ...
    <div class="h2-color-picker">
        ...
        >
            ...
            <svg
                ...
                :style="{display:suckerEnabled?'block':'none'}"
                class="sucker"
...
export default {
    ...
    methods: {
        ...
        mousemoveHandler(e) {
            ...
            const suckerMask = document.getElementById('suckerMaskId')
            if (pageX >= this.suckerArea[0] && pageY >= this.suckerArea[1] && pageX <= this.suckerArea[2] && pageY <= this.suckerArea[3]) {
                style.display = ''
                if (suckerMask !== null) {
                    suckerMask.css({
                        pointerEvents: 'none'
        ...
        inputColor(e) {
            const result = this.setColorValue(e.target.value)
            if (!result) {
                return
            }
            this.setColorPos()
        ...
        setColorValue(color) {
            ...
            const rgbaRegExp = new RegExp(/^((2[0-4][0-9]|25[0-5]|[01]?[0-9][0-9]?),){2}(2[0-4][0-9]|25[0-5]|[01]?[0-9][0-9]?),?(0\.\d{1,2}|1|0)$/)
        ...
        selectSaturation(e) {
            e.preventDefault()
            e.stopPropagation()
            if (e.target !== this.$refs.canvasSaturation) {
                return
            }
            const { top: saturationTop, left: saturationLeft } = this.$refs.saturation.getBoundingClientRect()
            const ctx = e.target.getContext('2d')
            const mousemove = e => {
                let x = e.pageX - saturationLeft
                let y = e.pageY - saturationTop
        ...
        // 绑定非吸色图层点击事件
        bindSuckerMaskClickEvent() {
            setTimeout(() => {
                const suckerMask = document.getElementById('suckerMaskId')
                if (suckerMask !== null) {
                    if (this.isOpenSucker) {
                        suckerMask.addEventListener('click', this.suckerMaskClick)
                    } else {
                        suckerMask.removeEventListener('click', this.suckerMaskClick)
                    }
                } else {
                    this.bindSuckerMaskClickEvent()
                }
