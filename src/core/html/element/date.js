...
export default class EqxDate extends EqxElement {
    constructor(elementJson, eqxPage) {
        ...
        if (!elementJson.id) {
            ...
            elementJson.type = elementType.date
    ...
    render() {
        super.render()
        const styleType = this.elementJson.property.type
        const $date = this.$date = document.createElement('div')
        let css = {
            left: '', // 清除多余的样式
            top: '',
            transform: '',
        ...
        if (styleType === textType.chartlet) {
        ...
        if (styleType === textType.gradient) {
            css = Object.assign(css, {
                webkitBackgroundClip: 'text',
                backgroundClip: 'text'
            })
        ...
        if ([textType.gradient, textType.chartlet].includes(styleType)) {
            display === 'none' ? $date.removeAttr('class') : $date.attr('class', 'gradientChartlet')
        }
    ...
    createInnerHTML() {
        const content = this.elementJson.property.content || '+yyyy'
        var date = new Date()
        var parse = dateParse(date, content)
        this.$date.innerHTML = parse
    }
    ...
    updateCss(css) {
        ...
        if (this.elementJson.css.writingMode && this.elementJson.css.writingMode === 'vertical-rl') {
            this.$date.css(css).css(uselessCss).css({
                // fontSize变化时，需要重新计算，并且需要先渲染
                width: ''
            })
            css.width = this.$date.offsetWidth - (parseInt(borderWidth) + parseInt(padding)) * 2 + 'px'
        } else {
            ...
            css.height = this.$date.offsetHeight - (parseInt(borderWidth) + parseInt(padding)) * 2 + 'px'
        }

        this.$date.css(css).css(uselessCss)
    }

    static content(content) {
        content = content || this.elementJson.property.content || '+yyyy'
        var date = new Date()
        var parse = dateParse(date, content)
        return parse
    }
    ...
    updateProperty(newProperty) {
        ...
        const { content } = newProperty
        const type = this.elementJson.property.type
        if (content) {
            this.$date.innerHTML = EqxDate.content(content)
        }
        ...
        switch (type) {
            ...
            case textType.stroke: {
                ...
                this.$date.css(uselessCss).css(EqxDate.specific(newPropertys))
                break
            }
            case textType.gradient:
                this.$date.css({
                    textShadow: ''
                }).css(EqxDate.dropShadow(newPropertys))
                    .css(EqxDate.strokeWithoutDistance(newProperty))
                    .css(EqxDate.gradient(newPropertys))
                break
            case textType.shake:
                this.$date.css(EqxDate.strokeWithoutDistance(newProperty))
                    .css(EqxDate.shake(newPropertys))
                break
            case textType.chartlet:
                this.$date.css({
                    ...
                    .css(EqxDate.strokeWithoutDistance(newProperty))
                    .css(EqxDate.chartlet(newPropertys))
                break
        }
        ...
        // 更新日期背景的圆角属性(注意：外层div和内层div都需要同时改变才行)
        this.$el.css(EqxDate.borderRadius(borderRadius))
        this.$date.css(EqxDate.borderRadius(borderRadius))
    }
}
